...
parser.add_argument('--index', type=int, help='Experiment Number', default='123')
parser.add_argument('--kfold', type=int, help='5 fold (0,1,2,3,4)',default='5')
...
valid_dataset1 = AudioDataset(config, root_dir=config.path['root_path'], dataset_names=(args.dataset1,), num_workers=1, preprocessing=False, train=False, kfold=args.kfold)
...
for epoch in range(restore_epoch, config.experiment['max_epoch']):
    ...
    print("training loss for %d epoch: %.4f" % (epoch + 1, np.mean(train_loss_list)))
    print("training accuracy for %d epoch: %.4f" % (epoch + 1, (correct.item() / total)))
    print("training top2 accuracy for %d epoch: %.4f" % (epoch + 1, ((correct.item() + second_correct.item()) / total)))
    ...
    with torch.no_grad():
        ...
        print("validation loss(%d): %.4f" % (epoch + 1, validation_loss))
        print("validation accuracy(%d): %.4f" % (epoch + 1, (val_correct.item() / val_total)))
        print("validation top2 accuracy(%d): %.4f" % (epoch + 1, ((val_correct.item() + val_second_correct.item()) / val_total)))
        ...
        if best_acc < val_correct.item() / val_total:
            ...
            print('==== best accuracy is %.4f and epoch is %d' % (best_acc, epoch + 1))
            print('saving model, Epoch %d, step %d' % (epoch + 1, current_step + 1))
    ...
    if (args.early_stop == True) and (early_stop_idx > 9):
        print('==== early stopped and epoch is %d' % (epoch + 1))
        break
    # learning rate decay
...
if os.path.isfile(os.path.join(asset_path, ckpt_path, ckpt_file_name % last_best_epoch)):
    ...
    model.load_state_dict(checkpoint['model'])
    print("restore model with %d epochs" % last_best_epoch)
else:
...
if args.voca == True:
    ...
    for m in score_metrics:
        average_score = (np.sum(song_length_list1) * average_score_dict1[m]) / (np.sum(song_length_list1))
        print('==== %s score 1 is %.4f' % (m, average_score_dict1[m]))
        # print('==== %s score 2 is %.4f' % (m, average_score_dict2[m]))
        # print('==== %s score 3 is %.4f' % (m, average_score_dict3[m]))
        print('==== %s mix average score is %.4f' % (m, average_score))
else:
