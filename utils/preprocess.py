...
class Preprocess():
    ...
    def get_all_files(self):
        ...
        if "isophonic" in self.dataset_names:
            ...
            label_paths = glob.glob("./chord_dataset/queen/chordlabs/**/*.lab",  recursive=True)
            audio_paths = glob.glob("./chord_dataset/queen/mp3/**/*.mp3",  recursive=True)            
            for label_path, audio_path in zip(label_paths, audio_paths):
                song_name = os.path.basename(audio_path).split(".mp3")[0].replace(" ", "_")
                res_list.append([song_name, label_path, audio_path, os.path.join(self.root_path, "result", "isophonic")])            
        ...
        for song_name, lab_path, mp3_path, save_path in all_list:
            ...
            for stretch_factor in stretch_factors:
                ...
                for shift_factor in shift_factors:
                    ...
                    x = librosa.time_stretch(original_wav, sr, stretch_factor)
                    x = librosa.pitch_shift(x, sr, shift_factor)
        ...
        for song_name, lab_path, mp3_path, save_path in all_list:
            ...
            for stretch_factor in stretch_factors:
                ...
                for shift_factor in shift_factors:
                    ...
                    k += 1
                    # stretch original sound and chord info
                    x = librosa.effects.pitch_shift(original_wav, sr, shift_factor)
                    audio_length = x.shape[0]
                    chord_info['start'] = chord_info['start'] * 1/stretch_factor
                    chord_info['end'] = chord_info['end'] * 1/stretch_factor
